...
@Injectable()
export class ClimaHttp extends AuthHttp {
  ...
  public patch(url: string, body: any, options?: RequestOptionsArgs): Observable<Response> {
    return this.fazerRequisicao(() => super.patch(url, body, options));
  }
  ...
  private fazerRequisicao(fn: Function): Observable<Response> {
    if (this.auth.isAccessTokenInvalido()) {
      ...
      return Observable.fromPromise(chamadaNovoAccessToken); // transforma para promesa pois cada método do http retorna um observable.
    } else {
      return fn();
    }
  }
